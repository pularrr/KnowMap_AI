...
export interface JobStore {
  save(job: StoredAgentJob, text: string): void;
  load(id: string): { job: StoredAgentJob; text: string } | undefined;
  list(sessionId: string): StoredAgentJob[];
  readText(id: string, sessionId: string, cursor: number, limit: number): { text: string; nextCursor: number | null; totalBytes: number; checksum: string };
}
...
export class FileJobStore implements JobStore {
  ...
  save(job: StoredAgentJob, text: string) {
  ...
  load(id: string) {
    ...
    return { job: { ...metadata, result, textBytes: Buffer.byteLength(text), textChecksum: metadata.textChecksum ?? createHash("sha256").update(text).digest("hex"), revision: metadata.revision ?? 1 }, text };
  }
  list(sessionId: string) {
    if (!existsSync(this.root)) return [];
    return readdirSync(this.root).filter((name) => /^[a-f0-9-]+\.json$/.test(name)).flatMap((name) => {
      try { const value = this.load(name.slice(0, -5)); return value?.job.sessionId === sessionId ? [value.job] : []; }
      catch { return []; }
    }).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  }
  readText(id: string, sessionId: string, cursor: number, limit: number) {
