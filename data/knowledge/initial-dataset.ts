...
export function createInitialDataset(): KnowledgeDataset {
  ...
  const nodes: KnowledgeNode[] = [{ id: root.id, canonicalName: root.name, shortFact: root.shortFact, aliases: [], nodeType: "domain", domainId: firstDomain.id as KnowledgeNode["domainId"], visualBranch: firstDomain.visualBranch as KnowledgeNode["visualBranch"], primaryParentId: null, level: 0, order: 0, tags: [], status: "draft" }, ...profile.domains.map((domain, index): KnowledgeNode => ({ id: domain.id, canonicalName: domain.name, shortFact: domain.description, aliases: [], nodeType: "domain", domainId: domain.id as KnowledgeNode["domainId"], visualBranch: domain.visualBranch as KnowledgeNode["visualBranch"], primaryParentId: root.id, level: 1, order: index + 1, tags: [], status: "draft" }))];
