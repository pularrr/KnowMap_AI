...
export async function startAgentJob(input: { id?: string; sessionId: string; nodeId: string; kind: AgentJob["kind"]; query: string; sourceText?: string; sourceKind?: "conversation" | "summary" | "paper" | "document" }): Promise<AgentJobSummary> {
  ...
  if (existing?.job.state === "cancelled") return { ...existing.job, result: undefined, hasResult: Boolean(existing.job.result) };
  ...
  void (async () => {
    ...
    try {
      ...
      if (input.kind === "chat") {
        ...
        const source = input.kind === "summary" ? await latestConversationSummary(input.sessionId) : input.sourceText?.trim();
      ...
      if (controller.signal.aborted || await cancellationRequested(job.id)) { job.state = "cancelled"; job.error = "任务已停止，已完成的内容仍保留。"; }
