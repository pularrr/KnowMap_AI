...
export async function createApp({ profile: profileId, profileFile, name, output }) {
  ...
  return withProjectModules(async load => {
    ...
    for (const folder of ["core", "server", "features", "plugin", "profiles", "scripts", "data/knowledge"]) {
    ...
    copyFileSync(join(projectRoot, "app/knowledge.ts"), join(target, "app/knowledge.ts"));
