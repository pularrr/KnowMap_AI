...
export function normalizeResearch(value: unknown): ResearchDocument {
  ...
  if (p && typeof p === "object" && !Array.isArray(p)) {
    ...
    if (Array.isArray(mapped.newNodes)) mapped.newNodes = mapped.newNodes.map((item: unknown) => {
      ...
      return {...n,canonicalName:n.canonicalName ?? n.canonical_name ?? n.name,shortFact:n.shortFact ?? n.short_fact ?? n.description ?? n.definition,
        parentId:n.parentId ?? n.primaryParentId ?? n.parent_id,nodeType:n.nodeType ?? n.node_type,
  ...
  const optional = new Set(["nodeId","id","parentId","nodeType","relationshipType","relationshipRationale","language","items","code","url","note"]);
...
{"answer":"研究小结","coverageAssessment":"已覆盖与缺口","converged":false,"gaps":["仍需探索的问题"],"proposal":{"summary":"摘要","rationale":"理由","newNodes":[{"id":"稳定英文或拼音ID","canonicalName":"具体名称","shortFact":"定义","nodeType":"concept|method|algorithm|model|problem|parameter|metric|application|phenomenon|component|artifact","parentId":"已有或本批新父节点ID","blocks":[{"type":"definition","title":"定义与边界","text":"完整知识"}]}],"cardBlocks":[{"nodeId":"节点ID","type":"principle","title":"原理","text":"完整知识"}],"relations":[{"sourceId":"节点ID","targetId":"节点ID","type":"PREREQUISITE_OF","rationale":"方向和理由"}],"evidence":[{"title":"真实来源或模型领域知识","url":"仅在已核实时填写","note":"来源支持什么；未经外部检索明确标注模型知识待核验"}]}}。
