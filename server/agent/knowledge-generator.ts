...
export class KnowledgeGenerator {
  ...
  initializeNetwork(): KnowledgeNetwork {
    ...
    return { nodes, cardBlocks: nodes.map(n => ({ nodeId: n.id, type: "definition", title: "定义与边界", text: n.shortFact })),
  ...
  async generate(options: { initialNetwork?: KnowledgeNetwork; confirmedMvp?: boolean; mvpAcceptanceToken?: string; signal?: AbortSignal;
    ...
    const dataset = networkToDataset(options.initialNetwork ?? this.initializeNetwork(), this.profile);
    const mvp = this.mode === "mvp";
    const rounds = Math.max(2, Math.min(3, this.profile.initialization.mvp.reactRounds[1]));
    ...
    const doc = await collectAdaptiveResearch({ provider: this.provider, dataset,
      ...
      runId: `gen-${crypto.randomUUID()}`, root: true, observations: [], profile: this.profile,
      signal: options.signal, onProgress: report, onStats: s => Object.assign(stats, s),
    ...
    const prepared = operationsFromResearch(doc, dataset, this.profile.initialization.rootNode.id);
